...
export const AddProduct: React.FC = () => {
  const { categories, addProduct, adminSettings } = useAppContext();
  ...
  const [category, setCategory] = useState(categories[0]?.slug || "men");
  const [price, setPrice] = useState<number>(50);
  const [stock, setStock] = useState<number>(20);
  const [featured, setFeatured] = useState(false);
  const [trending, setTrending] = useState(false);

  // Size list check boxes
  ...
  // Images uploaded Base64 string lists
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (images.length === 0) {
      // Inject standard placeholder if no images uploaded
      const placeholders = [
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[10px] text-neutral-500 uppercase tracking-widest font-bold">
            Add new inventory to showroom registry
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        {/* LEFT COLUMN: Basic Settings Form */}
        ...
        {/* RIGHT COLUMN: Colors / Images Form */}
